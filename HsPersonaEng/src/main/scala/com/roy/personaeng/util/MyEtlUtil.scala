...
object MyEtlUtil {
  ...
  def getGroupDataFromFile(tableJsonConfig: EtlJsonTableConfig, userName: String, runDate:String): RDD[(String, Iterable[Map[String, String]])] = {
    var filePath = tableJsonConfig.fileName.replace(Constant.runDateStr,runDate).replace("//","/")
    if(!filePath.startsWith("/")){
      filePath = "/"+filePath
    }
    filePath = Constant.HDFS_PATH+"/"+filePath
